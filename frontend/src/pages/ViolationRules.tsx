            ...
            <tbody>
              {rules.map(rule => (
                <tr key={rule.rule_id} style={{
                  ...
                  <td style={{ ...tdStyle, fontFamily: 'monospace', fontWeight: 600, color: '#333', whiteSpace: 'nowrap' }}>
                    {rule.rule_id}
                  </td>
                  <td style={tdStyle}>
                    <div style={{ fontWeight: 500 }}>{rule.title_he}</div>
                    <div style={{ color: '#888', fontSize: '0.8rem' }}>{rule.title_en}</div>
                    {rule.description_he && (
                      <div style={{ color: '#666', fontSize: '0.78rem', marginTop: 2 }}>{rule.description_he}</div>
                    )}
                  </td>
                  <td style={{ ...tdStyle, fontSize: '0.8rem', color: '#666', maxWidth: 200 }}>
                    {rule.legal_basis_he && <div>{rule.legal_basis_he}</div>}
                    {rule.legal_basis_en && <div style={{ color: '#aaa' }}>{rule.legal_basis_en}</div>}
                  </td>
                  <td style={{ ...tdStyle, whiteSpace: 'nowrap' }}>
                    {editing === rule.rule_id ? (
                      ...
                      <span
                        ...
                        style={{ cursor: 'pointer', borderBottom: '1px dashed #aaa', padding: '0 2px' }}
                        title="לחץ לעריכה"
                      >
                  ...
                  <td style={{ ...tdStyle, textAlign: 'center' }}>
                    <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                      ...
                      />
                      <span style={{ fontSize: '0.8rem', color: rule.is_active ? '#2a7a2a' : '#999' }}>
